...
#' module_water_electricity_water_ARG_xml
#'
#' Construct XML data structure for \code{electricity_water_ARG.xml}.
#'
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{electricity_water_ARG.xml}. The corresponding file in the
#' original data system was \code{batch_electricity_water_ARG.xml.R} (water XML).
module_water_electricity_water_ARG_xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c(XML = "electricity_water_ARG.xml"))
    ...
    L2233.StubTechCO2_elec_arg <- L2233.StubTech_elec_cool %>%
      filter(region == "Argentina") %>%
      repeat_add_columns(tibble(year = MODEL_YEARS)) %>%
      mutate(CO2 = "CO2_elec")

    L2233.GlobalTechCapture_elec_cool_arg <- L2233.GlobalTechCapture_elec_cool %>%
      mutate(target.gas = if_else(year >= 2035, "CO2_elec", "CO2"))

    # Produce outputs
    create_xml("electricity_water_ARG.xml") %>%
      ...
      add_xml_data(L2233.GlobalTechCapture_elec_cool_arg, "GlobalTechCaptureCO2") %>%
      ...
      add_xml_data(L2233.StubTechCO2_elec_arg, "StubTechCO2") %>%
      ...
      add_precursors("L223.Supplysector_elec",
                     ...
                     "L2233.GlobalIntTechCapital_elec_cool",
                     "L223.GlobalTechCapFac_elec") ->
      electricity_water_ARG.xml

    return_data(electricity_water_ARG.xml)
  } else {
